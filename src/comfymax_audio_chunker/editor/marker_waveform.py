...
class MarkerWaveform(Waveform):
    ...
    def paintEvent(self,event):
        ...
        top=23; bottom=self.height()-(8 if self.overview else 35)
        if self.context:
        ...
        for pixel in range(width):
            ...
            if first<len(low):
                p.drawLine(QPointF(pixel+8,center-min(1,float(np.max(high[first:last])))*amp),
                           QPointF(pixel+8,center-max(-1,float(np.min(low[first:last])))*amp))
        ...
        if self.overview:
            ...
            p.fillRect(QRectF(8,bottom+4,width,26),QColor('#30291e'))
            p.setPen(QColor('#e8bd73')); p.drawText(QRectF(12,bottom+4,80,24),Qt.AlignVCenter,'Markers')
        ...
        for i,frame in enumerate([0]+self.markers+[self.doc.data['timeline']['frames']]):
            ...
            if a<=seconds<=a+span:
                ...
                if not self.overview:
                    ...
                    p.drawText(QRectF(x+4,bottom+4,70,24),Qt.AlignVCenter,label)
    ...
    def mouseMoveEvent(self,event):
        ...
        self.setToolTip(f'{seconds:.6f} seconds')
